...
class PerPromptStatTracker:
    ...
    def update(self, prompts, rewards, type='grpo'):
        ...
        for prompt in unique:
            # 为计算暂时转换为数组，但不修改 self.stats[prompt]
            prompt_stats_array = np.array(self.stats[prompt])
            prompt_rewards = rewards[prompts == prompt]  # Fix: Recalculate prompt_rewards for each prompt
            mean = np.mean(prompt_stats_array, axis=0, keepdims=True)
            if self.global_std:
                std = np.std(rewards, axis=0, keepdims=True) + 1e-4  # Use global std of all rewards
            else:
                std = np.std(prompt_stats_array, axis=0, keepdims=True) + 1e-4
            if type=='grpo':

...
class PromptEditorPolicy(nn.Module):
    ...
    def __init__(self, 
                 ...
                 use_local_models: bool = False,  # Set to True for local models, False for HuggingFace
                 gtr_model_path: str = None,  # GTR model path (local or HuggingFace)
                 sbert_model_path: str = None,  # SBERT model path (local or HuggingFace)
        ...
        try:
            ...
            # Determine SBERT model path based on loading mode
            if use_local_models and sbert_model_path:
                sbert_model_name = sbert_model_path
                print(f"[INFO] Loading local SBERT model from: {sbert_model_name}")
            else:
                # Use HuggingFace model
                sbert_model_name = "sentence-transformers/all-MiniLM-L6-v2"
                print(f"[INFO] Loading HuggingFace SBERT model: {sbert_model_name}")
        ...
        # Determine GTR model path based on loading mode
        if use_local_models and gtr_model_path:
            gtr_model_name = gtr_model_path
            print(f"[INFO] Loading local GTR model from: {gtr_model_name}")
        else:
            # Use HuggingFace model
            gtr_model_name = "sentence-transformers/gtr-t5-base"
            print(f"[INFO] Loading HuggingFace GTR model: {gtr_model_name}")

...
def pipeline_with_logprob(
    ...
    # Handle DDP-wrapped transformer by accessing the module attribute
    transformer_config = getattr(self.transformer, 'module', self.transformer).config
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            # Apply step callback control if provided
            if step_callback is not None:
                # Create condition dictionary for callback
                cond = {
                    "prompt_embeds": prompt_embeds,
                    "pooled_prompt_embeds": pooled_prompt_embeds,
                    "timestep": t,
                ...
                u_t = step_callback(latents, float(t), cond)
                if u_t is not None:
                    # Inject control additively to the noise prediction (drift term)
                    noise_pred = noise_pred + u_t
